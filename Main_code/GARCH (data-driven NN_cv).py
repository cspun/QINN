...
class BSINNsquared:
    ...
    def net_sigma(self, x, t, v):
        sigma = self.neural_net(tf.concat([x,t,v],1), self.sigma_weights, self.sigma_biases)
        return sigma
...
if __name__ == "__main__": 
    ...
    # Train/test data
    infile = open('../train_test data/GARCH_4_folds_data_v_5000','rb')
    ...
    infile = open('../train_test data/GARCH_4_folds_data_s_v_5000','rb')
    data = pickle.load(infile)
    infile.close()
    ...
    for l in num_layers:
        for n in num_neurons:
            ...
            sigma_layers = np.concatenate([[3], n*np.ones(l), [1]]).astype(int).tolist()
            ...
            for i in range(k):
                ...
                filename = 'GARCH_%d_l_%d_n_%d_fold_nn_fitsolver_rep_v(with_train)'%(l,n,i+1)
                outfile = open(filename,'wb')
                pickle.dump(GARCH_data,outfile)
                outfile.close()
