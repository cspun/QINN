...
class BSINN:
    ...
    def __init__(self, X_1, X_test, u, u_test, Xf, Xf_test, uf, uf_test, 
        ...
        self.epsilon = tf.constant([1.], dtype=tf.float32) 
    ...
    def net_f(self, x, t, v):
        ...
        f = -u_t + (v/2)*x**2*u_xx + r*x*u_x + rho*epsilon*v**(3/2)*x*u_xv + (epsilon**2*v**2/2)*u_vv + kappa*(theta-v)*u_v - r*u
...
if __name__ == "__main__": 
    ...
    # Train/test data
    infile = open('GARCH_4_folds_data_v_5000','rb')
    ...
    infile = open('GARCH_4_folds_data_s_v_5000','rb')
    data = pickle.load(infile)
    infile.close()
    ...
    for l in num_layers:
        for n in num_neurons:
            ...
            for i in range(k):
                ...
                GARCH_data = {'X_u_test_1':X_u_test_1, 'X_star_1': X_star_1, 'X_test_tot':X_test_tot, 'u_test_pred':u_test_pred, 
                ...
                filename = 'GARCH_%d_l_%d_n_%d_fold_constant_fitsolver_rep_v(with_train)'%(l,n,i+1)
                outfile = open(filename,'wb')
                pickle.dump(GARCH_data_Ht,outfile)
                outfile.close()
