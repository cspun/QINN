    ...
    def train(self, nIter):
        ...
        for it in range(nIter):
            ...
            for p in range(len(batch_x)):
                ...
                #    print('It: %d, Loss: %.3e, sigma: %.3f, Time: %.2f' % 
                #          (it, loss_value, sigma_value, elapsed))
                #    start_time = time.time()
        ...
        for n in num_neurons:
            ...
            for alpha in np.linspace(0.25,0.75,3):
                ...
                for i in range(4):
                    ...
                    BS_data = {'X_u_test_1':X_u_test_1, 'X_star_1': X_star_1, 'X_test_tot':X_test_tot, 'u_test_pred':u_test_pred, 
                              'u_test_tot':u_test_tot, 'K_test_tot':K_test_tot,'alpha':alpha, 'sigma': sigma_value,
                              'X_train_tot':X_train_tot, 'u_train_pred':u_train_pred, 'u_train_tot':u_train_tot, 'K_train_tot':K_train_tot}
                    
                    filename = 'BS_%d_l_%d_n_%d_fold_constant_fitsolver_alpha_%d_rep(with_train)'%(l,n,i+1,c)
