...
class BSINNsquared:
    ...
    def __init__(self, X_1, X_test, u, u_test, 
                 Xf, Xf_test, uf, uf_test, 
                 Xb, Xb_test, ub, ub_test, 
                 Xi, Xi_test, ui, ui_test, 
                 u_layers, sigma_layers, alpha):
        ...
        #self.sigma = tf.Variable([.4], dtype=tf.float32)
    ...
    def net_f(self, x, t):
        ...
        r = self.r
        u = self.net_u(x,t)
        sigma = self.net_sigma(x,t)
        u_t = tf.gradients(u, t)[0]
        u_x = tf.gradients(u, x)[0]
        u_xx = tf.gradients(u_x, x)[0]
        f = -u_t + (sigma**2*(x**2)/2)*(u_xx) + r*x*u_x - r*u
        
        return f
    
    def net_sigma(self, x, t):
        sigma = self.neural_net(tf.concat([x,t],1), self.sigma_weights, self.sigma_biases)
        return sigma
        ...
        for it in range(nIter):
            ...
            for p in range(len(batch_x)):
                tf_dict = {self.x_tf: batch_x[p], self.t_tf: batch_t[p], self.u_tf: batch_u[p],
                           self.xf_tf: batch_x[p], self.tf_tf: batch_t[p], self.uf_tf: batch_u[p],
...
if __name__ == "__main__": 
    ...
    # Train/test data
    infile = open('BS_4_folds_data','rb')
    ...
    infile = open('BS_4_folds_data_s','rb')
    data = pickle.load(infile)
    infile.close()
    ...
    for l in num_layers:
        for n in num_neurons:
            ...
            sigma_layers = np.concatenate([[2], n*np.ones(l), [1]]).astype(int).tolist()
            ...
            for i in range(k):
                ...
                for alpha in np.linspace(0,1,5):
                    ...
                    for j in range(4):
                        ...
                        #total for u
                        X_u_test_1_s = Data_test_s[i][j][:,0:2]
                        u_test_s = Data_test_s[i][j][:,2][:,None]
                        K_test_s = Data_test_s[i][j][:,3][:,None]
                        
                        X_u_train_1_s = Data_train_s[i][j][:,0:2]
                        u_train_s = Data_train_s[i][j][:,2][:,None]
                        K_train_s = Data_train_s[i][j][:,3][:,None]
                        
                        # domain
                        X_f_test_1_s = Data_test_s[i][j][:,0:2]
                        uf_test_s = Data_test_s[i][j][:,2][:,None]
                        Kf_test_s = Data_test_s[i][j][:,3][:,None]
                        
                        X_f_train_1_s = Data_train_s[i][j][:,0:2]
                        uf_train_s = Data_train_s[i][j][:,2][:,None]
                        Kf_train_s = Data_train_s[i][j][:,3][:,None]
                        
                        # boundary
                        X_b_test_1_s = bd_test_s[i][j][:,0:2]
                        ub_test_s = bd_test_s[i][j][:,2][:,None]
                        Kb_test_s = bd_test_s[i][j][:,3][:,None]
                        
                        X_b_train_1_s = bd_train_s[i][j][:,0:2]
                        ub_train_s = bd_train_s[i][j][:,2][:,None]
                        Kb_train_s = bd_train_s[i][j][:,3][:,None]
                        
                        # initial
                        X_i_test_1_s = ini_test_s[i][j][:,0:2]
                        ui_test_s = ini_test_s[i][j][:,2][:,None]
                        Ki_test_s = ini_test_s[i][j][:,3][:,None]
                        
                        X_i_train_1_s = ini_train_s[i][j][:,0:2]
                        ui_train_s = ini_train_s[i][j][:,2][:,None]
                        Ki_train_s = ini_train_s[i][j][:,3][:,None]     
                        
                        X_test_tot_s = Data_test_tot_s[i][j][:,0:2]
                        u_test_tot_s = Data_test_tot_s[i][j][:,2:3]
                        K_test_tot_s = Data_test_tot_s[i][j][:,3][:,None]
                        
                        model = BSINNsquared(X_u_train_1_s, X_u_test_1_s, u_train_s, u_test_s,
                        ...
                        X_star_1_s = Data_train[i][:,0:2]
                        u_star_s = Data_train[i][:,2:3]
                        K_s = Data_train[i][:,3:4]
                ...
                BS_data = {'X_u_test_1':X_u_test_1, 'u_test_pred':u_test_pred, 'u_test_tot':u_test_tot, 
                              'K_test_tot':K_test_tot,'alpha':alpha_opt}
                filename = 'BS_%d_l_%d_n_%d_fold_nn_fitsolver_rep'%(l,n,i+1)
                outfile = open(filename,'wb')
                pickle.dump(BS_data,outfile)
                outfile.close()
