...
class BSINN:
    ...
    def __init__(self, X_1, X_test, u, u_test, Xf, Xf_test, uf, uf_test, 
        ...
        self.kappa = tf.constant([2], dtype=tf.float32)  #2
        self.theta = tf.constant([0.055], dtype=tf.float32) #0.055
        self.epsilon = tf.constant([0.6], dtype=tf.float32) #0.6
        self.rho = tf.constant([-0.7], dtype=tf.float32) #-0.7
    ...
    def net_f(self, x, t, v):
        ...
        f = -u_t + (v/2)*x**2*u_xx + r*x*u_x + rho*epsilon*v*x*u_xv + (epsilon**2*v/2)*u_vv + kappa*(theta-v)*u_v - r*u
...
if __name__ == "__main__": 
    ...
    # Train/test data
    infile = open('../train_test data/Ht_4_folds_data_v_5000','rb')
    ...
    infile = open('../train_test data/Ht_4_folds_data_s_v_5000','rb')
    data = pickle.load(infile)
    infile.close()
        ...
        for n in num_neurons:
            ...
            for i in range(k):
                ...
                for alpha in np.linspace(0,1,5):
                    ...
                    Total_error_u_num_s = 0
                    Total_error_u_den_s = 0
                    Total_error_u_test_num_s = 0
                    Total_error_u_test_den_s = 0
                    
                    for j in range(4):
                        
                        #total for u
                        X_u_test_1_s = Data_test_s[i][j][:,0:3]
                        u_test_s = Data_test_s[i][j][:,3][:,None]
                        K_test_s = Data_test_s[i][j][:,4][:,None]
                        
                        X_u_train_1_s = Data_train_s[i][j][:,0:3]
                        u_train_s = Data_train_s[i][j][:,3][:,None]
                        K_train_s = Data_train_s[i][j][:,4][:,None]
                        
                        # domain
                        X_f_test_1_s = Data_test_s[i][j][:,0:3]
                        uf_test_s = Data_test_s[i][j][:,3][:,None]
                        Kf_test_s = Data_test_s[i][j][:,4][:,None]
                        
                        X_f_train_1_s = Data_train_s[i][j][:,0:3]
                        uf_train_s = Data_train_s[i][j][:,3][:,None]
                        Kf_train_s = Data_train_s[i][j][:,4][:,None]
                        
                        # boundary
                        X_b_test_1_s = bd_test_s[i][j][:,0:3]
                        ub_test_s = bd_test_s[i][j][:,3][:,None]
                        Kb_test_s = bd_test_s[i][j][:,4][:,None]
                        
                        X_b_train_1_s = bd_train_s[i][j][:,0:3]
                        ub_train_s = bd_train_s[i][j][:,3][:,None]
                        Kb_train_s = bd_train_s[i][j][:,4][:,None]
                        
                        # initial
                        X_i_test_1_s = ini_test_s[i][j][:,0:3]
                        ui_test_s = ini_test_s[i][j][:,3][:,None]
                        Ki_test_s = ini_test_s[i][j][:,4][:,None]
                        
                        X_i_train_1_s = ini_train_s[i][j][:,0:3]
                        ui_train_s = ini_train_s[i][j][:,3][:,None]
                        Ki_train_s = ini_train_s[i][j][:,4][:,None]     
                        
                        X_test_tot_s = Data_test_tot_s[i][j][:,0:3]
                        u_test_tot_s = Data_test_tot_s[i][j][:,3:4]
                        K_test_tot_s = Data_test_tot_s[i][j][:,4][:,None]
                        
                        model = BSINN(X_u_train_1_s, X_u_test_1_s, u_train_s, u_test_s, 
                                                  X_f_train_1_s, X_f_test_1_s, uf_train_s, uf_test_s,
                                                  X_b_train_1_s, X_b_test_1_s, ub_train_s, ub_test_s, 
                                                  X_i_train_1_s, X_i_test_1_s, ui_train_s, ui_test_s,
                                                  u_layers, alpha)
                        model.train(5000)
                        
                        X_star_1_s = Data_train[i][:,0:3]
                        u_star_s = Data_train[i][:,3:4]
                        K_s = Data_train[i][:,4:5]
                        
                        u_pred = model.predict_all(X_star_1_s)
                        
                        error_u_num_s = (np.linalg.norm(u_star_s*K_s - u_pred*K_s,2))**2
                        error_u_den_s = (np.linalg.norm(u_star_s*K_s,2))**2
                        
                        
                        u_test_pred = model.predict_test(X_test_tot_s)
                        
                        # Truncate negative option price
                        u_min = np.maximum((X_test_tot_s[:,0:1]-np.exp(-model.r*X_test_tot_s[:,1:2])),0)
                        
                        
                        for q in range(len(u_test_pred)):
                            if u_test_pred[q] < u_min[q]:
                                u_test_pred[q] = u_min[q] # actually no need loop, just use maximum function
                                
                        error_u_test_num_s = (np.linalg.norm(u_test_tot_s*K_test_tot_s - u_test_pred*K_test_tot_s))**2
                        error_u_test_den_s = (np.linalg.norm(u_test_tot_s*K_test_tot_s))**2
                    
                        Total_error_u_num_s += error_u_num_s
                        Total_error_u_den_s += error_u_den_s
                        Total_error_u_test_num_s += error_u_test_num_s
                        Total_error_u_test_den_s += error_u_test_den_s
                        
                        print('%.4e'%np.sqrt(error_u_test_num_s/error_u_test_den_s))
                        print('%.4e'%np.sqrt(Total_error_u_test_num_s/Total_error_u_test_den_s))
                        
                    Total_error_u_test = np.sqrt(Total_error_u_test_num_s/Total_error_u_test_den_s)
                    
                    print('alpha: %.3f 4-fold completed for %dl%dn %dfold #######################################'%(alpha,l,n,i+1))
                    print('Total Error u: %.4e' % (np.sqrt(Total_error_u_num_s/Total_error_u_den_s)))
                    print('Total Error u_test: %.4e' %Total_error_u_test)
                    
                    if Total_error_u_test < error_min:
                        error_min = Total_error_u_test
                        alpha_opt = alpha
                
                
                
                
                print('error_min: %.4e'%error_min)
                ...
                u_min_star = np.maximum((X_star_1[:,0:1]-np.exp(-model.r*X_star_1[:,1:2])),0)
                
                for q in range(len(u_pred)):
                    if u_pred[q] < u_min_star[q]:
                        u_pred[q] = u_min_star[q]
                ...
                Ht_data = {'X_u_test_1':X_u_test_1, 'X_star_1': X_star_1, 'X_test_tot':X_test_tot, 'u_test_pred':u_test_pred, 
                            'u_test_tot':u_test_tot, 'K_test_tot':K_test_tot,'alpha':alpha, 'X_train_tot':X_train_tot, 
                           'u_train_pred':u_train_pred, 'u_train_tot':u_train_tot, 'K_train_tot':K_train_tot}
                filename = 'Ht_%d_l_%d_n_%d_fold_constant_fitsolver_rep_v(with_train)'%(l,n,i+1)
                outfile = open(filename,'wb')
                pickle.dump(Ht_data,outfile)
                outfile.close()
