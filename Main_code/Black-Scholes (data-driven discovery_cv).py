...
class BSINN:
    ...
    def __init__(self, X_1, X_test, u, u_test, 
        ...
        #self.sigma = tf.constant([.4], dtype=tf.float32)
        self.sigma = tf.Variable([0.1], dtype=tf.float32, constraint=lambda x: tf.clip_by_value(x, 0., 1.))
    ...
    for l in num_layers:
        for n in num_neurons:
            ...
            for i in range(k):
                ...
                sigma_value = model.sess.run(model.sigma)
                print('sigma : %.3f'%(sigma_value))
                
                BS_data = {'X_u_test_1':X_u_test_1, 'u_test_pred':u_test_pred, 'u_test_tot':u_test_tot, 
                              'K_test_tot':K_test_tot,'alpha':alpha_opt, 'sigma': sigma_value}
                filename = 'BS_%d_l_%d_n_fold_%d_variable_fitsolver_rep_1_revised'%(l,n,i+1)
